...
class TestFastGradientMethod(unittest.TestCase):
    def setUp(self):
        ...
        def my_model(x):
            h1 = tf.nn.sigmoid(tf.matmul(x, tf.constant([[1.5, .3], [-2, 0.3]], dtype=tf.float32)))
            res = tf.nn.softmax(tf.matmul(x, tf.constant([[-2.4, 1.2], [0.5, -2.3]], dtype=tf.float32)))
            return res
    ...
    def test_generate_np_gives_adversarial_example(self):
        ...
        for ord in [np.inf, 1, 2]:
            ...
            if ord == np.inf:
                ...
                assert np.allclose(np.sum(np.square(x_adv-x_val),axis=1)**.5, 0.5)
