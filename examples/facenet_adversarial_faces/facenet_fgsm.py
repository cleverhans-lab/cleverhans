...
class InceptionResnetV1Model(Model):
  ...
  def convert_to_classifier(self):
    ...
    self.victim_embedding_input = tf.placeholder(tf.float32, shape=(None, 128))
...
with tf.Graph().as_default():
  with tf.Session() as sess:
    ...
    accuracy = np.mean((np.argmax(labels, axis=-1)) == (
    ...
    accuracy = np.mean((np.argmax(labels[same_faces_index], axis=-1)) == (
        np.argmax(adversarial_labels[same_faces_index], axis=-1)))
    print('Accuracy against adversarial examples for ' +
          'same person faces (dodging): ' + str(accuracy * 100) + '%')

    accuracy = np.mean((np.argmax(labels[different_faces_index], axis=-1)) == (
        np.argmax(adversarial_labels[different_faces_index], axis=-1)))
    print(
        'Accuracy against adversarial examples for ' +
        'different people faces (impersonation): ' + str(accuracy * 100) + '%')
