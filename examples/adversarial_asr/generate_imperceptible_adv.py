...
flags.DEFINE_integer('num_iter_stage1', '1000',
...
class Attack:
  def __init__(self, sess, batch_size=1,
    ...
    with cluster, tf.device(cluster.GetPlacer()):
      ...
      self.apply_delta = tf.clip_by_value(
          self.delta, -FLAGS.initial_bound, FLAGS.initial_bound) * self.rescale
  ...
  def attack_stage2(self, audios, trans, adv, th_batch, psd_max_batch, maxlen, sample_rate, masks, masks_freq, num_loop, data, lr_stage2):
    ...
    sess.run(tf.assign(self.rescale, np.ones(
        (self.batch_size, 1), dtype=np.float32)))
    sess.run(tf.assign(self.alpha, np.ones(
        (self.batch_size), dtype=np.float32) * 0.05))
    ...
    for i in range(MAX):
      ...
      for ii in range(self.batch_size):
        ...
        if i % 10 == 0:
          ...
          if i % 100 == 0:
            print("example: {}".format(num_loop * self.batch_size + ii))
            print("iteration: %d, alpha: %f, loss_ce: %f, loss_th: %f" %
          ...
          if predictions['topk_decoded'][ii, 0] == trans[ii].lower():
            if l[ii] < loss_th[ii]:
              ...
              print(
                  "-------------------------------------Succeed---------------------------------")
              print("save the best example=%d at iteration= %d, alpha = %f" %
                    (ii, i, alpha[ii]))
    ...
    with tf.Session(config=tfconf) as sess:
      ...
      for l in range(num_loops):
        ...
        adv_example = attack.attack_stage1(
            audios, trans, th_batch, psd_max_batch, maxlen, sample_rate, masks, masks_freq, l, data_sub, FLAGS.lr_stage2)
        ...
        adv_example, loss_th, final_alpha = attack.attack_stage2(
            audios, trans, adv, th_batch, psd_max_batch, maxlen, sample_rate, masks, masks_freq, l, data_sub, FLAGS.lr_stage2)

        # save the adversarial examples in stage 2
        for i in range(batch_size):
          print("example: {}".format(i))
          print("Final distortion for stage 2: {}, final alpha is {}, final loss_th is {}".format(np.max(
              np.abs(adv_example[i][:lengths[i]] - audios[i, :lengths[i]])), final_alpha[i], loss_th[i]))
