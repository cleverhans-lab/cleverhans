...
flags.DEFINE_boolean(
    'adv', 'True', 'to test adversarial examples or clean examples')
  ...
  with tf.device("/gpu:0"):
    ...
    with tf.Session(config=tfconf) as sess:
      ...
      with cluster, tf.device(cluster.GetPlacer()):
        ...
        new_input = create_speech_rir(
            input_tf, rir_tf, lengths, maxlen, batch_size) * mask
        ...
        for num_room in range(FLAGS.num_test_rooms):
          ...
          for l in range(num_loops):
            ...
            audios_np, sample_rate, tgt_np, mask_freq, lengths_np, max_len, masks = Read_input(
                data_sub, batch_size)
