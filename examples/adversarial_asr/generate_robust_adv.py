...
flags.DEFINE_float('initial_bound', '2000.',
                   'initial l infinity norm for adversarial perturbation')
...
flags.DEFINE_integer('num_iter_stage1', '2000',
                     'number of iterations in stage 1')
flags.DEFINE_integer('num_iter_stage2', '4000',
...
def Readrir():
  ...
  _, rir = wav.read(FLAGS.root_dir + FLAGS.rir_dir +
                    "_rir_" + str(index) + ".wav")
  return rir
...
class Attack:
  def __init__(self, sess, batch_size=1,
    ...
    with cluster, tf.device(cluster.GetPlacer()):
      ...
      self.delta_large = tf.Variable(np.zeros(
          (batch_size, FLAGS.max_length_dataset), dtype=np.float32), name='qq_delta')
      ...
      self.mask = tf.placeholder(dtype=np.float32, shape=[
                                 batch_size, None], name='qq_mask')
      ...
      # variable
      self.rescale = tf.Variable(np.ones(
          (batch_size, 1), dtype=np.float32) * FLAGS.initial_bound, name='qq_rescale')

      # extract the delta
      self.delta = tf.slice(tf.identity(self.delta_large), [
                            0, 0], [batch_size, self.maxlen])
      self.apply_delta = tf.clip_by_value(
          self.delta, -self.rescale, self.rescale)
      self.before_rir = tf.clip_by_value(
          self.apply_delta * self.mask + self.input_tf, -2**15, 2**15-1)
      self.new_input = create_speech_rir(
          self.before_rir, self.rir, self.lengths, self.maxlen, self.batch_size) * self.mask
  ...
  def attack_stage1(self, audios, trans, maxlen, sample_rate, masks, masks_freq, num_loop, data, lengths):
    ...
    sess.run(tf.assign(self.rescale, np.ones(
        (self.batch_size, 1), dtype=np.float32) * FLAGS.initial_bound))
    sess.run(tf.assign(self.delta_large, np.zeros(
        (self.batch_size, FLAGS.max_length_dataset), dtype=np.float32)))
    ...
    for i in range(1, MAX + 1):
      ...
      if i % 10 == 0:
        d, apply_delta, cl, predictions, new_input = sess.run(
            (self.delta, self.apply_delta, self.celoss, self.decoded, self.new_input), feed_dict)
      ...
      for ii in range(self.batch_size):
        ...
        if i % 10 == 0:
          if predictions['topk_decoded'][ii, 0] == trans[ii].lower():
            ...
            print("Stage 1: save the example at iteration i=%d example ii=%d celoss=%f bound=%f" % (
                i, ii, cl[ii], rescale[ii]))
        ...
        if i % 10 == 0:
          for counter in range(num_rooms[ii]):
            ...
            if sum_counter == num_counters[ii]:
              print("-------------------------------True--------------------------")
              print(" The num_counter is %d for example %d" %
                    (num_counters[ii], ii))
              num_counters[ii] += 1
              ...
              final_adv[ii] = new_input[ii]
              final_perturb[ii] = apply_delta[ii]
              print("Stage 2: save the example at iteration i=%d example ii=%d celoss=%f" % (
                  i, ii, cl[ii]))
  ...
  with tf.device("/gpu:0"):
    ...
    with tf.Session(config=tfconf) as sess:
      ...
      for l in range(num_loops):
        ...
        audios, trans, maxlen, sample_rate, masks, masks_freq, lengths = ReadFromWav(
            data_sub, batch_size)
        adv_example, perturb = attack.attack_stage1(
            audios, trans, maxlen, sample_rate, masks, masks_freq, l, data_sub, lengths)

        # save the adversarial examples in stage 1 that can only successfully attack one simulated room
        for i in range(batch_size):
          print("Final distortion for stage 1", np.max(
              np.abs(adv_example[i][:lengths[i]] - audios[i, :lengths[i]])))
          name, _ = data_sub[0, i].split(".")
          saved_name = FLAGS.root_dir + \
              str(name) + "_robust_speechrir_stage1.wav"
          adv_example_float = adv_example[i] / 32768.
          wav.write(saved_name, 16000, np.array(
              np.clip(adv_example_float[:lengths[i]], -2**15, 2**15-1)))

          saved_name = FLAGS.root_dir + \
              str(name) + "_robust_perturb_stage1.wav"
          perturb_float = perturb[i] / 32768.
          wav.write(saved_name, 16000, np.array(
              np.clip(perturb_float[:lengths[i]], -2**15, 2**15-1)))
          print(saved_name)
        ...
        adv_example, perturb = attack.attack_stage2(
            audios, trans, adv, rescales, maxlen, sample_rate, masks, masks_freq, l, data_sub, lengths)

        # save the adversarial examples in stage 2 that can successfully attack a set of simulated rooms
        for i in range(batch_size):
          print("Final distortion for stage 2", np.max(
              np.abs(adv_example[i][:lengths[i]] - audios[i, :lengths[i]])))
          name, _ = data_sub[0, i].split(".")
          saved_name = FLAGS.root_dir + \
              str(name) + "_robust_speechrir_stage2.wav"
          adv_example_float = adv_example[i] / 32768.
          wav.write(saved_name, 16000, np.array(
              np.clip(adv_example_float[:lengths[i]], -2**15, 2**15-1)))

          saved_name = FLAGS.root_dir + \
              str(name) + "_robust_perturb_stage2.wav"
          perturb_float = perturb[i] / 32768.
          wav.write(saved_name, 16000, np.array(
              np.clip(perturb_float[:lengths[i]], -2**15, 2**15-1)))
          print(saved_name)
