...
class TrainManager(object):
  ...
  def _init_eval(self):
    ...
    self.evaluate = Evaluator(self.sess, self.model, self.batch_size, x_pre, x,
                              y, self.data, self.writer, self.hparams)
...
class TrainerMultiGPU(TrainManager):
  ...
  def _create_train_graph(self):
    ...
    inputs, outputs = self.clone_g0_inputs_on_ngpus(inputs, outputs,
                                                    self.g0_inputs)
