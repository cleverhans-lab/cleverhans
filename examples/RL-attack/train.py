...
from rlattack.common.misc_util import (
    boolean_flag, pickle_load, pretty_eta, relatively_safe_pickle_dump,
...
def parse_args():
  ...
  # Bells and whistles
  boolean_flag(
      parser, "noisy", default=False, help="whether or not to NoisyNetwork")
  boolean_flag(
      parser,
      "double-q",
      default=True,
...
if __name__ == '__main__':
  ...
  with U.make_session(4) as sess:
    ...
    exploration = PiecewiseSchedule(
        [(0, 1.0), (approximate_num_iters / 50, 0.1),
    ...
    while True:
      ...
      if args.noisy:
        ...
        # epsilon greedy
        action = act(
            np.array(obs)[None], update_eps=exploration.value(num_iters))[0]
      ...
      if (num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20)
        ...
        td_errors, huber_loss = train(obses_t, actions, rewards, obses_tp1,
                                      dones, weights)
        ...
        im_stats.add_all_summary(
            ...
            [action, rew, np.mean(td_errors),
             np.mean(huber_loss)] + mean_sigma, num_iters)
      ...
      if num_iters > 0 and (num_iters % args.save_freq == 0 or
                            info["steps"] > args.num_steps):
        maybe_save_model(
            savedir, container, {
      ...
      if done:
        ...
        fps_estimate = (float(steps_per_iter) /
                        (float(iteration_time_est) + 1e-6) if
                        steps_per_iter._value is not None else "calculating:")
        logger.dump_tabular()
        logger.log()
