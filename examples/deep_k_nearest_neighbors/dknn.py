...
class DkNNModel(Model):
  ...
  def init_lsh(self):
    ...
    self.centers = {}  # mean of training data representation per layer (that needs to be substracted before LSH).
  ...
  def find_train_knns(self, data_activations):
    ...
    for layer in self.layers:
      ...
      for i in range(data_activations_layer.shape[0]):
        query_res = self.query_objects[layer].find_k_nearest_neighbors(data_activations_layer[i], self.neighbors)
        try:
  ...
  def fprop_np(self, data_np):
    ...
    if not self.calibrated:
      raise Error("DkNN needs to be calibrated by calling DkNNModel.calibrate method once before inferring.")
  ...
  def calibrate(self, cali_data, cali_labels):
    ...
    assert all([v.shape == (self.nb_cali, self.neighbors) for v in cali_knns_ind.itervalues()])
    assert all([v.shape == (self.nb_cali, self.neighbors) for v in cali_knns_labels.itervalues()])
...
def dknn_tutorial():
  ...
  with tf.Session() as sess:
    with tf.variable_scope('dknn'):
      ...
      # Define a model.
      nb_filters = 64
      model = make_basic_picklable_cnn(nb_filters=64, nb_classes=10, input_shape=(None, 28, 28, 1))
      preds = model.get_logits(x)
      loss = CrossEntropy(model, smoothing=0.)

      # Define the test set accuracy evaluation.
      def evaluate():
        acc = model_eval(sess, x, y, preds, x_test, y_test, args={'batch_size': FLAGS.batch_size})
        print('Test accuracy on test examples: %0.4f' % acc)

      # Train the model
      train_params = {'nb_epochs': FLAGS.nb_epochs, 'batch_size': FLAGS.batch_size, 'learning_rate': FLAGS.lr}
      ...
      def get_activations(data):
        ...
        for layer in layers:
          ...
          data_activations[layer] = batch_eval(sess, [x], [layer_sym], [data], args={'batch_size': FLAGS.batch_size})[0]
      ...
      dknn = DkNNModel(FLAGS.neighbors, layers, get_activations, train_data, train_labels, nb_classes, scope='dknn')
      ...
      for data_in, fname in zip([test_data, adv], ['test', 'adv']):
        ...
        plot_reliability_diagram(dknn_preds, np.argmax(y_test, axis=1), '/tmp/dknn_' + fname + '.pdf')
...
if __name__ == '__main__':
  ...
  flags.DEFINE_integer('nb_cali', 750, 'Number of calibration points for the DkNN')
  flags.DEFINE_integer('neighbors', 75, 'Number of neighbors per layer for the DkNN')
