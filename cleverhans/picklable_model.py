...
class Conv2D(Layer):
  ...
  def fprop(self, x, **kwargs):
    ...
    out = tf.nn.conv2d(x, self.kernels.var, (1,) + tuple(self.strides) + (1,),
                       self.padding)
    if self.use_bias:
      out = out + self.b.var
    return out
...
class Print(Layer):
  ...
  def fprop(self, x, **kwargs):
    ...
    return tf.Print(
        x,
        [tf.reduce_min(x), mean, tf.reduce_max(x), std], "Print layer")
...
class PerImageStandardize(Layer):
  ...
  def fprop(self, x, **kwargs):
    ...
    with tf.device("/CPU:0"):
      out = tf.map_fn(lambda ex: tf.image.per_image_standardization(ex), x)
    return out
...
class GroupNorm(Layer):
  ...
  def __init__(self, num_groups=32, eps=1e-3, init_gamma=1., **kwargs):
...
class BatchNorm(Layer):
  ...
  def __init__(self, eps=1e-3, init_gamma=1., **kwargs):
    self.eps = eps
    self.init_gamma = init_gamma
  ...
  def set_input_shape(self, shape):
    ...
    self.gamma = PV(init_value, name=self.name + "_gamma")
    self.beta = PV(
        np.zeros((channels,), dtype='float32'), name=self.name + "_beta")
