  ...
  def fprop(self, x, **kwargs):
    ...
    with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
      ...
      for scale in range(log_resolution - 2):
        y = tf.compat.v1.layers.conv2d(y, self.nb_filters << scale, **conv_args)
        y = tf.compat.v1.layers.conv2d(y, self.nb_filters << (scale + 1), **conv_args)
        y = tf.compat.v1.layers.average_pooling2d(y, 2, 2)
      y = tf.compat.v1.layers.conv2d(y, self.nb_classes, **conv_args)
      logits = tf.reduce_mean(input_tensor=y, axis=[1, 2])
