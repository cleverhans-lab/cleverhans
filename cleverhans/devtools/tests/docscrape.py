...
class NumpyDocString(object):
  ...
  def _parse(self):
    ...
    for (section, content) in self._read_sections():
      ...
      if section in ('Parameters', 'Other Parameters', 'Returns', 'Raises',
                     'Warns', 'Attributes', 'Methods'):
  ...
  def __str__(self):
    ...
    for param_list in ('Parameters', 'Other Parameters', 'Returns', 'Raises',
                       'Warns'):
  ...
  def get_errors(self, check_order=True):
    ...
    if check_order:
      canonical_order = [
          'Signature', 'Summary', 'Extended Summary', 'Attributes', 'Methods',
          'Parameters', 'Other Parameters', 'Returns', 'Raises', 'Warns',
          'See Also', 'Notes', 'References', 'Examples', 'index'
      ]
      ...
      for s in self.section_order:
        while canonical_order_copy and s != canonical_order_copy[0]:
          ...
          if not canonical_order_copy:
            errors.append("Sections in wrong order (starting at %s). The"
...
class NumpyClassDocString(NumpyDocString):
  ...
  def __init__(self, docstring, class_name, class_object):
    ...
    methods = dict(
        (name, func) for name, func in inspect.getmembers(class_object))
  ...
  def get_errors(self):
    errors = NumpyDocString.get_errors(self)
    if not self['Parameters'] and self.has_parameters:
      errors.append("%s class has no Parameters section" % self.class_name)
    return errors
...
class FunctionDoc(object):
  ...
  def __str__(self):
    ...
    if doc['Signature']:
      out += '%s\n' % header('**%s**' % doc['Signature'].replace('*', '\\*'),
                             '-')
    else:
      try:
...
def handle_method(method, method_name, class_name):
  ...
  if docstring is None:
    ...
    method_errors = [
        ...
        for e in NumpyFunctionDocString(docstring, method).get_errors()
    ]
  return method_errors
...
def handle_class(val, class_name):
  ...
  if docstring is None:
    ...
    cls_errors = [
        (e,)
        for e in NumpyClassDocString(docstring, class_name, val).get_errors()
    ]
...
def docstring_errors(filename, global_dict=None):
  ...
  try:
    ...
    raise AssertionError(
        "Couldn't verify format of " + filename + "due to SkipTest")
  all_errors = []
