...
def test_format_pep8():
  ...
  config_file = os.path.abspath(
      os.path.join(cleverhans_path, os.pardir,
                   "cleverhans/devtools/tests/pycodestyle.sty"))
  pep8_checker = StyleGuide(config_file=config_file)
  files_to_check = []
...
def verify_format_docstrings():
  ...
  for path in list_files(".py"):
    ...
    try:
      ...
      format_infractions.append([
          "%s failed to run so format cannot "
