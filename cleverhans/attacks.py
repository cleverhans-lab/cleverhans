...
class Attack(object):
  ...
  def construct_variables(self, kwargs):
    ...
    if not all(
        isinstance(value, collections.Hashable) for value in fixed.values()):
...
class FastGradientMethod(Attack):
  ...
  def generate(self, x, **kwargs):
    ...
    :param x: The model's symbolic inputs.
    :param eps: (required float) attack perturbation magnitude (input
                variation)
  ...
  def parse_params(self,
    ...
    :param eps: (required float) attack perturbation magnitude (input
                variation)
    :param ord: (optional) Order of the perturbation's norm
                (mimics NumPy). Possible values: np.inf, 1 or 2.
    :param y: (optional) A tensor with the model labels. Only provide
              this parameter if you'd like to use true labels when crafting
              adversarial samples. Otherwise, model predictions are used as
              labels to avoid the "label leaking" effect (explained in this
    ...
    assert eps is not None, "`eps` is a required parameter"
    self.eps = eps
    self.ord = ord
    self.y = y
...
class BasicIterativeMethod(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.structural_kwargs = ['ord', 'step_count']
  ...
  def parse_params(self,
    ...
    # Save attack-specific parameters
    assert eps is not None, "`eps` is a required parameter"
    assert step_size is not None, "`step_size` is a required parameter"
...
class MomentumIterativeMethod(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.structural_kwargs = ['ord', 'step_count', 'decay_factor']
  ...
  def generate(self, x, **kwargs):
    ...
    def body(i, ax, m):
      ...
      scaled_grad = self.step_size * normalized_grad
      ax = ax + scaled_grad
...
class VirtualAdversarialMethod(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.structural_kwargs = ['step_count']
...
class ElasticNetMethod(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.structural_kwargs = [
        'beta', 'decision_rule', 'batch_size', 'confidence', 'targeted',
  ...
  def generate(self, x, **kwargs):
    ...
    attack = EAD(self.sess, self.model, self.beta, self.decision_rule,
                 self.batch_size, self.confidence, 'y_target' in kwargs,
                 self.learning_rate, self.binary_search_steps,
                 self.max_iterations, self.abort_early, self.initial_const,
  ...
  def generate(self, x, **kwargs):
    ...
    def deepfool_wrap(x_val):
      return deepfool_batch(self.sess, x, preds, logits, grads, x_val,
                            self.nb_candidate, self.overshoot, self.max_iter,
...
class LBFGS(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.feedable_kwargs = {'y_target': self.tf_dtype}
    self.structural_kwargs = [
        'batch_size', 'binary_search_steps', 'max_iterations', 'initial_const',
        'clip_min', 'clip_max'
    ]
  ...
  def generate(self, x, **kwargs):
    ...
    attack = LBFGS_attack(self.sess, x, self.model.get_probs(x), self.y_target,
                          self.binary_search_steps, self.max_iterations,
                          self.initial_const, self.clip_min, self.clip_max,
                          nb_classes, self.batch_size)
...
class MadryEtAl(Attack):
  ...
  def __init__(self, model, back='tf', sess=None, dtypestr='float32'):
    ...
    self.structural_kwargs = ['ord', 'step_count', 'rand_init']
  ...
  def attack_single_step(self, x, eta, y):
    ...
    scaled_signed_grad = self.step_size * tf.sign(grad)
