...
def fgsm(x, predictions, eps=0.3, clip_min=None, clip_max=None):
    """
    Keras implementation of the Fast Gradient
    Sign method.
    ...
    """
    def output(merged_input):
        x, predictions = merged_input
        # Compute loss
        mask = K.equal(predictions, K.max(predictions, axis=1, keepdims=True))
        y = K.cast(mask, dtype=floatX)
        y = y / K.sum(y, 1, keepdims=True)

        loss = utils.model_loss(y, predictions, mean=True)
    ...
    def output_shape(merged_input_shape):
        return merged_input_shape[0]

    return merge([x, predictions], mode=output, output_shape=output_shape)
