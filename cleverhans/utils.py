...
known_number_types = (int, float, np.float16, np.float32, np.float64, np.int8,
                      np.int16, np.int32, np.int32, np.int64, np.uint8,
...
def get_logits_over_interval(sess,
  ...
  x = tf.placeholder(
      dtype=tf.float32, shape=[1, height, width, channels], name='x')
  # Define adv_x
...
def linear_extrapolation_plot(log_prob_adv_array,
                              y,
                              file_name,
                              min_epsilon=-10,
                              max_epsilon=10,
