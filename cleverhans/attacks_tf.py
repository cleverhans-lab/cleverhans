...
class ElasticNetMethod(object):
    ...
    def attack(self, imgs, targets):
        ...
        if len(images) % batch_size != 0:
            last_elements = len(images) - (len(images) % batch_size)
            _logger.debug(("Running EAD attack on instance " +
                           "{} of {}").format(last_elements, len(imgs)))
            temp_images = np.zeros((batch_size,) + images.shape[2:])
            temp_targets = np.zeros((batch_size,) + targets.shape[2:])
            temp_images[:(len(images) % batch_size)] = imgs[last_elements:]
            temp_targets[:(len(images) % batch_size)] = targets[last_elements:]
            temp_data = self.attack_batch(temp_images, temp_targets)
            r.extend(temp_data[:(len(images) % batch_size)],
