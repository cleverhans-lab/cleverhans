...
class ElasticNetMethod_EN(object):
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                l, l2s, l1s, elastic, scores, nimg = self.sess.run([self.loss,
                                                         self.l2dist,
                                                         self.l1dist,
                                                         self.elasticdist,
                                                         self.output,
            ...
            _logger.debug(" Elastic Mean successful distortion: {:.4g}".format(mean))

        # return the best solution found
        o_besten = np.array(o_besten)
        return o_bestattack

class ElasticNetMethod_L1(object):
    ...
    def __init__(self, sess, model, beta,
                 ...
                 clip_min, clip_max, num_labels, shape):
        """
        EAD Attack with the L1 Decision Rule
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                ...
                for e, (l1, sc, ii) in enumerate(zip(l1s, scores, nimg)):
                    lab = np.argmax(batchlab[e])
                    if l1 < bestl1[e] and compare(sc, lab):
                        bestl1[e] = l1
                        bestscore[e] = np.argmax(sc)
                    if l1 < o_bestl1[e] and compare(sc, lab):
                        o_bestl1[e] = l1
                        o_bestscore[e] = np.argmax(sc)
                        o_bestattack[e] = ii
            ...
            mean = np.mean(np.sqrt(o_bestl1[o_bestl1 < 1e9]))
