...
def vatm(model,
  ...
  :param logits: the model's unnormalized output tensor (the input to
                 the softmax layer)
  :param eps: the epsilon (input variation parameter)
  :param step_count: number of attack iterations.
...
def jsma(sess,
  ...
  while (current != target and iteration < max_iters and
    ...
    if iteration % ((max_iters + 1) // 5) == 0 and iteration > 0:
      _logger.debug("Iteration {} of {}".format(iteration, int(max_iters)))
...
class CarliniWagnerL2(object):
  ...
  def __init__(self, sess, model, batch_size, confidence, targeted,
               ...
               initial_const, clip_min, clip_max, num_labels, shape):
    """
  ...
  def attack(self, imgs, targets):
    ...
    for i in range(0, len(imgs), self.batch_size):
      _logger.debug(("Running CWL2 attack on instance " + "{} of {}").format(
          i, len(imgs)))
      r.extend(
...
class ElasticNetMethod(object):
  ...
  def __init__(self, sess, model, beta, decision_rule, batch_size, confidence,
    ...
    other = reduce_max((1 - self.tlab) * self.output - (self.tlab * 10000), 1)
    other_y = reduce_max((1 - self.tlab) * self.output_y - (self.tlab * 10000),
                         1)
  ...
  def attack(self, imgs, targets):
    ...
    for i in range(0, len(imgs) // batch_size):
      _logger.debug(("Running EAD attack on instance " + "{} of {}").format(
    ...
    for outer_step in range(self.BINARY_SEARCH_STEPS):
      ...
      for iteration in range(self.MAX_ITERATIONS):
        ...
        l, l2s, l1s, crit, scores, nimg = self.sess.run([
            self.loss, self.l2dist, self.l1dist, self.crit, self.output,
            self.newimg
        ])
        if iteration % ((self.MAX_ITERATIONS // 10) or 1) == 0:
          _logger.debug(("    Iteration {} of {}: loss={:.3g} " +
                         "l2={:.3g} l1={:.3g} f={:.3g}").format(
                             iteration, self.MAX_ITERATIONS, l, np.mean(l2s),
                             np.mean(l1s), np.mean(scores)))
...
class LBFGS_attack(object):
  ...
  def __init__(self, sess, x, model_preds, targeted_label, binary_search_steps,
    ...
    self.shape = shape = tuple(
        [self.batch_size] + list(self.x.get_shape().as_list()[1:]))
  ...
  def attack(self, x_val, targets):
    ...
    for outer_step in range(self.binary_search_steps):
      ...
      preds = np.atleast_1d(
          utils_tf.model_argmax(self.sess, self.x, self.model_preds, adv_x))
...
class UnrolledAdam(UnrolledOptimizer):
  ...
  def _apply_gradients(self, grads, x, optim_state):
    ...
    for i in xrange(len(x)):
      ...
      new_optim_state["u"][i] = (self._beta2 * u_old +
                                 (1. - self._beta2) * g * g)
...
def pgd_attack(loss_fn,
  ...
  Args:
      ...
      :param eps: float, the L-infinity norm of the maximum allowable
                                      perturbation
      :param step_count: int, the number of steps of gradient descent
  ...
  if is_debug:
    with tf.device("/cpu:0"):
      input_image = tf.Print(input_image, [],
                             "Starting PGD attack with epsilon: %s" % eps)

  init_perturbation = tf.random_uniform(
      tf.shape(input_image), minval=-eps, maxval=eps, dtype=tf_dtype)
  init_perturbation = project_perturbation(init_perturbation, eps, input_image)
  ...
  def loop_body(i, perturbation, flat_optim_state):
    ...
    with tf.control_dependencies([loss]):
      ...
      if early_stop_loss_threshold:
        i = tf.cond(
            tf.less(loss, early_stop_loss_threshold), lambda: float(num_steps),
            lambda: i)
    return i + 1, projected_perturbation, nest.flatten(new_optim_state)

  def cond(i, *_):
    return tf.less(i, step_count)
