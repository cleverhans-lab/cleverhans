...
class Optimization(object):
  ...
  def __init__(self, dual_formulation_object, sess, optimization_params):
    ...
    self.projected_dual_object = self.project_dual (dual_object)
  ...
  def project_dual (self):
    ...
    projected_lambda_quad = [tf.identity(x) for x in self.dual_object.lambda_quad]
    ...
    if self.dual_object.nn_params.num_hidden_layers == 1:
      ...
      matrix = (
          tf.matmul(tf.matmul(tf.transpose(
              self.dual_object.nn_params.weights[self.dual_object.nn_params.num_hidden_layers-1]),
                              utils.diag(diag_entries)),
                    self.dual_object.nn_params.weights[self.dual_object.nn_params.num_hidden_layers-1]))
      new_matrix = utils.diag(
          2*self.dual_object.lambda_lu[self.dual_object.nn_params.num_hidden_layers - 1]) - matrix
      # Making symmetric
